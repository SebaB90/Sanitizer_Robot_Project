...
class EnergyNavigation (Node):
    ...
    def energy_evaluation(self):
        ...
        power_matrix_mW = [(i)/9.8*10 for i in self.power_matrix]
        ...
        if self.pub_time  is not None:
            ...
            if dt>0:
                if self.first_time_flag is True :
                    ...
                    self.energy_matrix = [0] * len(power_matrix_mW)

                self.energy_matrix = [min(98, int(self.energy_matrix[i] + 4*dt*power_matrix_mW[i])) for i in range(len(power_matrix_mW))]
